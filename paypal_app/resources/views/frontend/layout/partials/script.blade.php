<!-- jQuery-V1.12.4 -->
<script src="{{asset('/')}}frontend/asset/js/vendor/jquery-1.12.4.min.js"></script>
<!-- Popper js -->
<script src="{{asset('/')}}frontend/asset/js/vendor/popper.min.js"></script>
<!-- Bootstrap V4.1.3 Fremwork js -->
<script src="{{asset('/')}}frontend/asset/js/bootstrap.min.js"></script>
<!-- Ajax Mail js -->
<script src="{{asset('/')}}frontend/asset/js/ajax-mail.js"></script>
<!-- Meanmenu js -->
<script src="{{asset('/')}}frontend/asset/js/jquery.meanmenu.min.js"></script>
<!-- Wow.min js -->
<script src="{{asset('/')}}frontend/asset/js/wow.min.js"></script>
<!-- Slick Carousel js -->
<script src="{{asset('/')}}frontend/asset/js/slick.min.js"></script>
<!-- Owl Carousel-2 js -->
<script src="{{asset('/')}}frontend/asset/js/owl.carousel.min.js"></script>
<!-- Magnific popup js -->
<script src="{{asset('/')}}frontend/asset/js/jquery.magnific-popup.min.js"></script>
<!-- Isotope js -->
<script src="{{asset('/')}}frontend/asset/js/isotope.pkgd.min.js"></script>
<!-- Imagesloaded js -->
<script src="{{asset('/')}}frontend/asset/js/imagesloaded.pkgd.min.js"></script>
<!-- Mixitup js -->
<script src="{{asset('/')}}frontend/asset/js/jquery.mixitup.min.js"></script>
<!-- Countdown -->
<script src="{{asset('/')}}frontend/asset/js/jquery.countdown.min.js"></script>
<!-- Counterup -->
<script src="{{asset('/')}}frontend/asset/js/jquery.counterup.min.js"></script>
<!-- Waypoints -->
<script src="{{asset('/')}}frontend/asset/js/waypoints.min.js"></script>
<!-- Barrating -->
<script src="{{asset('/')}}frontend/asset/js/jquery.barrating.min.js"></script>
<!-- Jquery-ui -->
<script src="{{asset('/')}}frontend/asset/js/jquery-ui.min.js"></script>
<!-- Venobox -->
<script src="{{asset('/')}}frontend/asset/js/venobox.min.js"></script>
<!-- Nice Select js -->
<script src="{{asset('/')}}frontend/asset/js/jquery.nice-select.min.js"></script>
<!-- ScrollUp js -->
<script src="{{asset('/')}}frontend/asset/js/scrollUp.min.js"></script>
<!-- Main/Activator js -->
<script src="{{asset('/')}}frontend/asset/js/main.js"></script>